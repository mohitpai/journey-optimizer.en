...
* If you have messages that are not used in journeys and that you want to keep, save them as templates. Refer to this [page](../design/email-templates.md#save-as-template). Note that you will still be able to access them until deprecation.
...
For the automated phase, refer to this [page](../rn/inline-messages.md#process). For the manual phase, here are the actions to perform on the **production sandbox**:
...
Check for any errors on the automatically migrated live journeys in the status report ([learn more](../rn/inline-messages.md#status). Click the **Check status** button in the top banner.
...
Publish your new journey version. Your previous live version then moves to the "Closed" status.
...
Click the **Check status** button in the top banner ([learn more](../rn/inline-messages.md#status) and check that there has been no error during the automatic migration and that there is nothing left to migrate. Be aware that any journey in error (with messages) will be deprecated after September 5 (on all sandboxes).
...
The second iteration occurs at night-time between August 1st and August 2nd.
...
If you haven't published newer journey versions (see this [section](../rn/inline-messages-steps.md#migration-step-2-3)) in time meaning before iteration 2 (August 1st), then publish the newer version.

>[!NOTE]
>
>Stop the previous version or you will lose it and its associated reporting. 
