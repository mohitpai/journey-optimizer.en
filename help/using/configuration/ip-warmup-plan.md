...
* Here you can download the blank Excel [IP warmup plan template](assets/ip-warmup-plan-template.zip) to fill in.

* You can also download a [sample IP warmup plan](assets/ip-warmup-plan-sample.zip) already filled in with some data you can use as an example.
...
1. Upload the Excel file containing your IP warmup plan. [Learn more](#prepare-file)
    ...
    >[!NOTE]
    >
    >In case the upload fails, make sure you are using the correct formatting and file format (.xls or .xlsx). Use the [template](assets/ip-warmup-plan-template.zip) provided to you by Adobe.
