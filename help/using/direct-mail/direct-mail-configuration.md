...
We no longer create multiple files for direct mail, nor do we have a record threshold count. Note: we do remove duplicate rows if all the values in the row are the same.
