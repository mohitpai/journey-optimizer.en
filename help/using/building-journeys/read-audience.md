...
1. Set the **[!UICONTROL Throttling rate]**. This is the maximum number of profiles that can enter the read audience per second. This rate applies only to this activity. It does not apply to other activities in the journey. If you want to define a throttling rate on custom actions, for example, you need to use the throttling API. Refer to this [page](../configuration/throttling.md).
    ...
    >[!NOTE]
    >
    >The overall throttling rate per sandbox is set to 20,000 profiles per second. Therefore, the throttling rate of all the read audiences that run simultaneously in the same sandbox add up to at most 20,000 profiles per second. You cannot modify this cap.
